...
const s=await ext.storage.local.get(['mode','apiKey','workerUrl','workerToken','profile']);
...
$('settings').onsubmit=async e=>{e.preventDefault();try{
 ...
 if($('mode').value==='worker'){
  ...
  const allowed=await ext.permissions.request({origins:[u.origin+'/*']});if(!allowed)throw new Error('Workerへの接続権限が必要です。');workerUrl=u.href;
 }else if(!$('apiKey').value.trim())throw new Error('APIキーを入力してください。');
 await ext.storage.local.set({mode:$('mode').value,apiKey:$('apiKey').value.trim(),workerUrl,workerToken:$('workerToken').value.trim(),profile});$('status').textContent='保存しました。Webページで拡張機能のボタンを押してください。';
